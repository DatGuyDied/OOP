...
void *TAllocationBlock::allocate() {
  ...
  if(_free_count > 0)
  {
      ...
      _free_count--;
      std::cout << "TAllocationBlock: Allocate " << (_max_count - _free_count) << " of " << _count << std::endl;
  } else
  {
...
void *TAllocationBlock::reallocate() {
  void *result = nullptr;
  _max_count = _max_count * 2;
  _used_blocks = (char*)malloc(_size*_max_count);
  _free_blocks = (void**)malloc(sizeof(void*)*_max_count);
  for (size_t i = 0; i < _max_count; i++) {
    _free_blocks[i] = _used_blocks + i * _size;
  }
  _free_count = _max_count / 2;
